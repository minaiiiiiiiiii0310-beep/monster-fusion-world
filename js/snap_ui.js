...
const SnapUI = (() => {
    ...
    root.innerHTML = `
      <div class="snap-screen">
        <div class="snap-top">
          ...
          <div class="snap-info">
            <span class="snap-turn">ターン ${G.turn} / ${SnapEngine.MAX_TURN}</span>
  ...
  function renderResult() {
    ...
    let title = win ? '🎉 勝利！' : tie ? '🤝 引き分け' : '😢 敗北';
    const lanes = `${G.allyLanes || 0} - ${G.enemyLanes || 0}`;
    ...
    root.innerHTML = `
      <div class="snap-screen snap-result">
        <h1 class="${win ? 'win' : tie ? 'tie' : 'lose'}">${title}</h1>
  ...
  function snap() {
    // MVP: ベット倍率を増やす UI のみ（ロジックは Phase 3）
    if (typeof SoundFX !== 'undefined') SoundFX.sfx('crit');
    alert('Snap! ベット倍率を2倍に（ランクマッチ実装時に有効化）');
  }

  return {
    start, render, restart, exit,
    pickCard, dropOn, unplayCard, endTurn, snap,
  };
})();
